...
function Perk({ icon, title, description }: PerkProps) {
  return (
    <div className="flex gap-4 items-start">
      <div className="w-9 h-9 rounded-[10px] bg-violet/15 border border-violet/30 flex items-center justify-center text-violet flex-shrink-0">{icon}</div>
      <div>
        <h4 className="font-grotesk text-[15px] font-bold mb-1 text-text">{title}</h4>
        <p className="text-[13px] text-text2 leading-[1.5]">{description}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div id="partners" className="bg-bg3 border-t border-border border-b px-12 py-25 max-md:px-5 max-md:py-15">
      <div className="max-w-[1200px] mx-auto grid grid-cols-2 gap-20 items-start max-md:grid-cols-1 max-md:gap-10">
        <div>
          <div className="text-[11px] font-semibold tracking-[2px] uppercase text-neon mb-3">Partner program</div>
        ...
        <div className="bg-bg border border-border rounded-[24px] p-9 flex flex-col items-center justify-center text-center min-h-[280px] gap-4">
          ...
          <h3 className="font-grotesk text-[24px] font-bold text-neon tracking-[-0.5px]">Coming soon</h3>
